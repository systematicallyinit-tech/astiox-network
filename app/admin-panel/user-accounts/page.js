...
import { FiCheckCircle, FiPause, FiPlay, FiSearch } from "react-icons/fi";
import {
    FiEdit,
    FiTrash2,
    FiPauseCircle,
...
useEffect(() => {
  ...
  async function updateWalletAmount (values) {
    setIsMenuOpen(false);
        ...
        const handleDelete = async (transaction) => {
          try {
            ...
            const res = await axios.delete(
              `/api/auth/admin/getAllUsers/user?id=${transaction._id}`,
              {
                withCredentials: true,
                headers: { "Content-Type": "application/json" },
              }
      ...
      const handlePlay = async  (transaction) => {
          try {
            setLoading(true);
            const res = await axios.patch(
              `/api/auth/admin/getAllUsers/toggle-profit/users?userId=${user._id}`,
              {
                userID: transaction._id,
                isPlay: false
              },
      ...
      const handlePause = async  (transaction) => {
          try {
            setLoading(true);
            const res = await axios.patch(
              `/api/auth/admin/getAllUsers/toggle-profit/users?userId=${user._id}`,
    ...
    const toggleMenu = (transaction) => {
      if(transaction) {
        //initialValueAmount.balance = transaction.balance;
        //initialValueAmount.earnings = transaction.earnings;
                  ...
                  <table className="min-w-[1900px] w-full text-sm">
                    ...
                    <thead className="bg-gray-100 dark:bg-isoDark2 dark:text-white">
                      ...
                      <tr>
                        ...
                        <th className="px-5 py-4 text-left">Total Balance ($)</th>

                        <th className="px-5 py-4 text-left">Total Earnings ($)</th>
