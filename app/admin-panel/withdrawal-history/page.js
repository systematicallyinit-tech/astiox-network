      ...
      async function fetchTransactions() {
        try {
          ...
          const res = await axios.get(
            `/api/auth/admin/transactions/tran?type=withdrawal`,
            {
        ...
        const handleDelete = async (transaction) => {
          try {
            ...
            const res = await axios.delete(
              `/api/auth/admin/transactions/tran?id=${transaction._id}`,
              {
                withCredentials: true,
                headers: { "Content-Type": "application/json" },
              }
      ...
      const handleConfirm = async  (transaction) => {
          try {
            ...
            const res = await axios.patch(
              `/api/auth/admin/transactions/tran?userId=${user._id}`,
              {
                  ...
                  <table className="min-w-[1900px] w-full text-sm">
                    ...
                    <thead className="bg-gray-100 dark:bg-isoDark2 dark:text-white">
                      ...
                      <tr>
                        ...
                        <th className="px-5 py-4 text-left">Withdrawal Wallet Address</th>
                      ...
                      {filteredTransactions.length > 0 ? (
                        ...
                        filteredTransactions.map((transaction) => (
                          ...
                          >
                            ...
                            <td className="px-5 py-4 whitespace-nowrap text-red-500 font-semibold">
                              $
                            ...
                            <td className="px-5 py-4 whitespace-nowrap">
                              {transaction.plan?.plan_duration} Hours
                            </td>
                            ...
                            <td className="px-5 py-4 whitespace-nowrap font-medium">
                              {
                                transaction.currency === "BTC" ? 
                                  `${transaction.userId.wallet_address.bitcoin_BTC}` || "N/A"
                                : transaction.currency === "ETH" ?
                                  `${transaction.userId.wallet_address.ethereum_ETH}` || "N/A"
                                : transaction.currency === "USDT" ?
                                  `${transaction.userId.wallet_address.tether_USDT}` || "N/A"
                                : transaction.currency === "BNB" ?
                                  `${transaction.userId.wallet_address.binance_BNB}` || "N/A" 
                                : (<span className="flex items-center space-x-1"><span>{transaction.userId.wallet_address.tether_USDT}</span> <span className="text-xs text-isoColor1 dark:text-isoColor2">(USDT)</span></span>) || "N/A"
                              }
                            </td>
